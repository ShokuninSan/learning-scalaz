...
object CanTruthy {
  ...
  def truthys[A](f: A => Boolean): CanTruthy[A] =
    new CanTruthy[A] {
...
object CanTruthyControls {

  def truthyIf[A:CanTruthy, B, C](a: A)(trueCase: => B)(falseCase: => C) =
    if (implicitly[CanTruthy[A]].truthys(a)) trueCase else falseCase
